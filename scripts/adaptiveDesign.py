...
def adaptive_m_tl(para):
    ...
    for i in range(1, statesLib.shape[0]):
        calStates=stIntegration(simState_minimize,deltaT,u,[para[0],1499.97571,1.26647953,1.73352047,1.78063348,para[1],para[2]])
...
def adaptive_m_br(para):
    ...
    simStateLib_minimize[0]=simState_minimize
    for i in range(1, statesLib.shape[0]):
        calStates=stIntegration(simState_minimize,deltaT,u,[para[0],1791.59689,1.19716101,1.18141797,2.17190332,para[1],para[2]])
        simStateLib_minimize[i] = calStates
        simState_minimize = calStates
    e=statesLib-simStateLib_minimize
    eSum = np.linalg.norm(e, ord=2)
